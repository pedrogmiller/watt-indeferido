...
## Gate 1 implementado (console)

- Progresso = `project_phase` (não mês civil). Play = próximo interrupt (landing fase-elegível ou deadline).
- Schedule v2: `landings` 0..6 (= seed M0–M6) + `months` mirror. Ver `MVP-PLAY.md`.
- PHASE_BUMP conservador: land / pip / aia apenas. Sem auto obra/cod.
- HUD cartão Ourique: fase + próxima data crítica.
